...
exports.createHoneyProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files['image'] && req.files['image'][0]) {
      // FIX: Use the exact destination path generated by your middleware
      mainImageUrl = req.files['image'][0].destinationPath;
    }

    let galleryImageUrls = [];
    if (req.files && req.files['galleryImages']) {
      // FIX: Use destinationPath for gallery images too
      galleryImageUrls = req.files['galleryImages'].map((file) => file.destinationPath);
    }
...
exports.getHoneyProducts = async (req, res) => {
  try {
    ...
    if (minRating && minRating !== 'All Ratings' && !isNaN(Number(minRating))) query.rating = { $gte: Number(minRating) };
    if (featured === 'Featured' || featured === 'true' || featured === true) query.featured = true;
    else if (featured === 'Non-Featured' || featured === 'false' || featured === false) query.featured = false;
    ...
    res.status(500).json({ success: false, message: error.message || 'Server error fetching honey products' });
  }
};
...
exports.getHoneyProductById = async (req, res) => {
  try {
    ...
    if (!paramId) return res.status(400).json({ success: false, message: 'Product ID or slug is required' });

    const isObjectId = mongoose.Types.ObjectId.isValid(paramId);
    const product = isObjectId ? await HoneyProduct.findById(paramId) : await HoneyProduct.findOne({ slug: paramId });
...
exports.updateHoneyProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files['image'] && req.files['image'][0]) {
      // FIX: Use middleware destinationPath
      updates.image = req.files['image'][0].destinationPath;
    }

    if (req.files && req.files['galleryImages']) {
      const newGallery = req.files['galleryImages'].map((file) => file.destinationPath);
...
exports.deleteHoneyProduct = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Honey product deleted successfully' });
  } catch (error) {
