...
// SVG Bee Component
const BeeSVG = () => (
  <svg viewBox="0 0 64 64" fill="none" className="faqBreadcrumb-bee-svg">
...
// SVG Flower Element
const FlowerSVG = () => (
  <svg viewBox="0 0 100 100" fill="none" className="faqBreadcrumb-flower-svg">
...
const FaqBreadcrumb = () => {
  ...
  return (
    ...
    >
      {/* Dark Forest Overlay */}
      <div className="faqBreadcrumb-overlay"></div>

      {/* Floating Fog Effect */}
      <div className="faqBreadcrumb-fog fog-1"></div>
      <div className="faqBreadcrumb-fog fog-2"></div>
      <div className="faqBreadcrumb-fog fog-3"></div>

      {/* Parallax Content Wrapper */}
      <div className="faqBreadcrumb-parallax" ref={parallaxRef}>
        
        {/* Floating Golden Light Particles */}
        <div className="faqBreadcrumb-particles">
          {Array.from({ length: 35 }).map((_, i) => (
            <span 
              key={i} 
              className="faqBreadcrumb-particle" 
              style={{
        ...
        {/* Floating Leaves */}
        <div className="faqBreadcrumb-leaves">
          {Array.from({ length: 10 }).map((_, i) => (
            <span 
              key={i}
              className="faqBreadcrumb-leaf"
              style={{
                left: `${Math.random() * 100}%`,
        ...
        {/* Flying Bees */}
        <div className="faqBreadcrumb-bee bee-1" ref={(el) => (beesRef.current[0] = el)}>
          <BeeSVG />
        </div>
        <div className="faqBreadcrumb-bee bee-2" ref={(el) => (beesRef.current[1] = el)}>
          <BeeSVG />
        </div>
        <div className="faqBreadcrumb-bee bee-3" ref={(el) => (beesRef.current[2] = el)}>
          <BeeSVG />
        </div>
        <div className="faqBreadcrumb-bee bee-4" ref={(el) => (beesRef.current[3] = el)}>
          <BeeSVG />
        </div>
        ...
        <div className="faqBreadcrumb-content">
          ...
          <div className="faqBreadcrumb-honey-drops">
            <span className="faqBreadcrumb-drop drop-1"></span>
            <span className="faqBreadcrumb-drop drop-2"></span>
            <span className="faqBreadcrumb-drop drop-3"></span>
          </div>

          {/* Glassmorphic Breadcrumb Navigation Bar */}
          <motion.nav 
            className="faqBreadcrumb-nav"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.7 }}
          >
            <Link to="/" className="faqBreadcrumb-link">Home</Link>
            <FaChevronRight className="faqBreadcrumb-arrow" />
            <span className="faqBreadcrumb-current">FAQ</span>
          </motion.nav>

        </div>

        {/* Decorative Flowers & Honeycomb Elements */}
        <div className="faqBreadcrumb-flower left">
          <FlowerSVG />
        </div>
        <div className="faqBreadcrumb-flower right">
          <FlowerSVG />
        </div>

        <div className="faqBreadcrumb-honeycomb">
